<template>
<div class="bg-white bottom-0 rounded-t-2xl shadow-[5px_-5px_10px_rgba(0,0,0,0.2)]">
  <div class="p-2.5">
      ...
      <form @submit.prevent="updateUserDetails()" class="px-2 text-xl w-full">
        ...
        <div class="flex justify-between gap-2 py-2">
          <label>City: </label>
          <input type="text" class="outline-none  w-full" :class="{'input-right':!editAddress, 'editable-input': editAddress}" v-model="user.city" required :readonly="!editAddress">
        </div>

        <div class="flex justify-between gap-2 py-2">
          <label>Address: </label>
          <input type="text" class="outline-none w-full" :class="{'input-right':!editAddress, 'editable-input': editAddress}" v-model="user.street" required :readonly="!editAddress">
        </div>

        <div class="flex justify-between gap-2 py-2">
          <label>Psc: </label>
          <input type="text" class="outline-none w-full" :class="{'input-right':!editAddress, 'editable-input': editAddress}" v-model.number="user.psc" required :readonly="!editAddress">
        </div>
